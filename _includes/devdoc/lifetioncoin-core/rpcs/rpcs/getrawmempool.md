...
- n: "→ →<br>`fee`"
  t: "amount (Lifetioncoin)"
  p: "Required<br>(exactly 1)"
  d: "The transaction fee paid by the transaction in decimal Lifetioncoin"

- n: "→ →<br>`modifiedfee`"
  t: "amount (Lifetioncoin)"
  p: "Required<br>(exactly 1)"
  d: "*Added in Bitcoin Core 0.12.0*<br><br>The transaction fee with fee deltas used for mining priority in decimal Lifetioncoin"
...
- n: "→ →<br>`ancestorcount`"
  t: "number (int)"
  p: "Required<br>(exactly 1)"
  d: "*Added in Lifetioncoin Core 0.12.3 / Bitcoin Core 0.13.0*<br><br>The number of in-mempool ancestor transactions (including this one)"

- n: "→ →<br>`ancestorsize`"
  t: "number (int)"
  p: "Required<br>(exactly 1)"
  d: "*Added in Lifetioncoin Core 0.12.3 / Bitcoin Core 0.13.0*<br><br>The size of in-mempool ancestors (including this one)"

- n: "→ →<br>`ancestorfees`"
  t: "number (int)"
  p: "Required<br>(exactly 1)"
  d: "*Added in Lifetioncoin Core 0.12.3 / Bitcoin Core 0.13.0*<br><br>The modified fees (see `modifiedfee` above) of in-mempool ancestors (including this one)"
...
- n: "→ →<br>`instantsend`"
  t: "bool"
  p: "Required<br>(exactly 1)"
  d: "*Added in Lifetioncoin Core 0.12.3*<br><br>Set to `true` for transactions broadcast via the `ix` message (InstantSend lock requested). Set to `false` for standard (non-InstantSend) transactions"

- n: "→ →<br>`instantlock`"
  t: "bool"
  p: "Required<br>(exactly 1)"
  d: "*Added in Lifetioncoin Core 0.12.3*<br><br>Set to `true` for locked InstantSend transactions (masternode quorum has locked the transaction inputs via `txlvote` messages). Set to `false` if the masternodes have not approved the InstantSend transaction"
