...
- n: "→<br>`version`"
  t: "number"
  p: "Required<br>(exactly 1)"
  d: "This node's version of Lifetioncoin Core in its internal integer format.  For example, Lifetioncoin Core 0.12.2 has the integer version number 120200"
...
- n: "→ → →<br>`limited`"
  t: "bool"
  p: "Required<br>(exactly 1)"
  d: "Set to `true` if only connections to this network<!--noref--> are allowed according to the `-onlynet` Lifetioncoin Core command-line/configuration-file parameter.  Otherwise set to `false`"
...
- n: "→<br>`incrementalfee`"
  t: "number (DASH)"
  p: "Required<br>(exactly 1)"
  d: "*Added in Lifetioncoin Core 0.12.3*<br><br>The minimum fee increment for mempool limiting or BIP 125 replacement in DASH/kB"
...
{
  "version": 120300,
  "subversion": "/Lifetioncoin Core:0.12.3/",
