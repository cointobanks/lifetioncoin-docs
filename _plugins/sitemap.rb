  ...
  class SitemapGenerator < Generator
      ...
      File.open(File.join(site.dest, 'sitemap.xml'), 'w+') do |sitemap|
        ...
        locs['en']['url'].each do |id,value|
          locs.each do |lang,value|
            ...
            sitemap.puts '<url>'
            sitemap.puts '  <loc>https://cointobanks.github.io/lifetioncoin-docs/'+lang+'/'+CGI::escape(locs[lang]['url'][id])+'</loc>'
            locs.each do |altlang,value|
              ...
              sitemap.puts '    href="https://cointobanks.github.io/lifetioncoin-docs/'+altlang+'/'+CGI::escape(locs[altlang]['url'][id])+'" />'
            end
            sitemap.puts '</url>'
          end
        end
	#Add static non-translated pages
        Dir.glob('en/**/*.{md,html}').concat(Dir.glob('*.{md,html}')).each do |file|
          ...
          sitemap.puts '<url>'
          sitemap.puts '  <loc>https://cointobanks.github.io/lifetioncoin-docs/'+file.gsub('.html','').gsub('.md','')+'</loc>'
          sitemap.puts '</url>'
        end
        #Close sitemap
        sitemap.puts '</urlset>'
