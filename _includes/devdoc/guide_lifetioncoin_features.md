...
Lifetioncoin aims to be the most user-friendly and scalable payments-focused
cryptocurrency in the world. The Lifetioncoin network features instant transaction
...
While Lifetioncoin is based on Bitcoin and compatible with many key components of the
...
documentation describes these key features that set Lifetioncoin apart in the blockchain
economy.
...
Lifetioncoin Core's InstantSend feature provides a way to lock transaction inputs and
enable secure, instantaneous transactions. Since Lifetioncoin Core 0.13.0, any qualifying
...
including the InstantSend vulnerability that was fixed in Lifetioncoin Core 0.12.2.
Some specific points in the video are listed here for quick reference:
 ...
 - 9:00 - Description of Lifetioncoin Core pre-0.12.2 InstantSend vulnerability
 - 13:00 - Description of vulnerability fix / Post Lifetioncoin Core 0.12.2 operation
...
* Mempool usage must be lower than 10% (`AUTO_IX_MEMPOOL_THRESHOLD`). As of Lifetioncoin Core 0.13.0, this corresponds to a mempool size of 30 MB (`DEFAULT_MAX_MEMPOOL_SIZE` = 300 MB).

**Historical Note**

Prior to Lifetioncoin Core 0.13.0, `instantsend` and `instantlock` values were not
...
The introduction of Long-Living Masternode Quorums in Lifetioncoin Core 0.14 provides
...
output of the locking process is relayed to the entire Lifetioncoin network.
...
Lifetioncoin's ChainLock feature leverages [LLMQ Signing Requests/Sessions](#llmq<!--noref-->-signing-session)
...
Lifetioncoin Core's PrivateSend feature provides a way to improve privacy by performing
...
integer value used in P2P messages, and the actual Lifetioncoin value.
...
In Lifetioncoin Core, collaterals are created with enough value to pay 4 collateral fees
(4 x 0.001 DASH). ([Lifetioncoin Core Reference](https://github.com/dashpay/dash/blob/262454791c4b4f783b2533d1b16b757a71eb5f7d/src/privatesend<!--noref-->.h#L413))
...
4x the minimum collateral). In protocol versions > 70208, Lifetioncoin Core can use any
...
**Additional notes**
  ...
  * Lifetioncoin Core attempts to join an existing mixing pool 2/3 of the time although this is not a requirement that alternative implementations would be required to follow ([Lifetioncoin Core Reference](https://github.com/dashpay/dash/blob/e596762ca22d703a79c6880a9d3edb1c7c972fd3/src/privatesend<!--noref-->-client.cpp#L817-#L826))
  ...
  * Once the masternode has received valid `dsa` messages from the appropriate number of clients (`nSessionMaxParticipants`), it sends a `dsq` message with the ready bit set
    * Clients must respond to the Queue ready within 30 seconds or risk forfeiting the collateral they provided in the `dsa` message (Step 1) ([Lifetioncoin Core Reference](https://github.com/dashpay/dash/blob/e9f7142ed01c0d7b53ef8b5f6f3f6375a68ef422/src/privatesend<!--noref-->.h#L23))
  ...
  * Each client can provide up to 9 (`PRIVATESEND_ENTRY_MAX_SIZE`) inputs (and an equal number of outputs) to be mixed ([Lifetioncoin Core Reference](https://github.com/dashpay/dash/blob/e9f7142ed01c0d7b53ef8b5f6f3f6375a68ef422/src/privatesend<!--noref-->.h#L29))
  * This is the only message in the PrivateSend process that contains enough information to link a wallet's PrivateSend inputs with its outputs
    * This message is sent directly between a client and the mixing masternode (not relayed across the Lifetioncoin network) so no other clients see it
  ...
  * Once the masternode has received valid `dsi` messages from all clients, it creates the final transaction and sends a `dsf` message
    * Inputs/outputs are ordered deterministically as defined by [BIP-69](https://github.com/dashevo/bips/blob/master/bip-0069.mediawiki#Abstract) to avoid leaking any data ([Lifetioncoin Core Reference](https://github.com/dashpay/dash/blob/e596762ca22d703a79c6880a9d3edb1c7c972fd3/src/privatesend<!--noref-->-server.cpp#L321-#L322))
    * Clients must sign their inputs to the Final Transaction within 15 seconds or risk forfeiting the collateral they provided in the `dsi` message (Step 4) ([Lifetioncoin Core Reference](https://github.com/dashpay/dash/blob/e9f7142ed01c0d7b53ef8b5f6f3f6375a68ef422/src/privatesend<!--noref-->.h#L24))
...
* If mixing completes successfully, Lifetioncoin Core charges the collateral randomly in 1/10 mixing transactions to pay miners ([Lifetioncoin Core Reference](https://github.com/dashpay/dash/blob/e596762ca22d703a79c6880a9d3edb1c7c972fd3/src/privatesend<!--noref-->-server.cpp#L458-#L478))
* Clients that abuse the mixing system by failing to respond to `dsq` messages or `dsf` messages within the timeout periods may forfeit their collateral. Lifetioncoin Core charges the abuse fee in 2/3 cases ([Lifetioncoin Core Reference](https://github.com/dashpay/dash/blob/e596762ca22d703a79c6880a9d3edb1c7c972fd3/src/privatesend<!--noref-->-server.cpp#L397-#L398))
...
Since DIP3 (introduced in Lifetioncoin Core 0.13.0), masternode reward payments are based
...
Lifetioncoin Core performs full masternode synchronization as required. There are
several conditions that initiate a start/restart the sync process:

* Initial startup of Lifetioncoin Core
...
used prior to the deterministic masternode list update in Lifetioncoin Core v0.13 that
implemented DIP3.**
...
Prior to the deterministic masternode system introduced by DIP3 in Lifetioncoin Core 0.13,
...
Lifetioncoin Core synchronizes the governance system via the Masternode network as the
...
message is required for each object. Lifetioncoin Core periodically (~ every 6 seconds)
...
Lifetioncoin 12.1+ governance objects and tasks. Sentinel abstracts some governance
details away from Lifetioncoin Core for easier extensibility of the governance system in
the future. This will allow the integration between Evolution and Lifetioncoin Core to
proceed more smoothly and enable new governance object additions with minimal
impact to Lifetioncoin Core.
...
![Warning icon](/img/icons/icon_warning.svg) NOTE: Deprecated by [Lifetioncoin Core v0.14](https://github.com/dashpay/sentinel<!--noref-->/pull/64)

In Lifetioncoin Core 12.2, use of the `watchdog` governance object type was replaced
...
From Lifetioncoin Core 0.12.2 through Lifetioncoin Core 0.13, Sentinel used the `sentinelping` RPC
...
Lifetioncoin's masternode quorums are used to facilitate the operation of masternode provided
...
Lifetioncoin Core 0.14 (protocol version 70214) introduced the Long Living Masternode
...
to enable quorums to sign arbitrary messages. For example, Lifetioncoin Core 0.14 uses
...
Note the following timeouts defined by Lifetioncoin Core related to signing sessions:
...
| MN Broadcast | 10      | Majority | _Deprecated by DIP3 (deterministic masternode list) in Lifetioncoin Core 0.13._<br><br>If a majority (6+) of nodes agree, a new `mnb` message is not required.
...
service requirements in support of the future Lifetioncoin Platform (Evolution)
functionality.
