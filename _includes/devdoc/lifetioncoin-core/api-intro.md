...
Lifetioncoin Core RPCs accept and return the byte-wise reverse of computed
...
However, Lifetioncoin Core's RPCs use the byte-wise reverse for hashes, so if you
...
Lifetioncoin Core's use of hashes (which are byte arrays in C++) as integers
...
other Lifetioncoin software, this reversal of hashes has become the standard
